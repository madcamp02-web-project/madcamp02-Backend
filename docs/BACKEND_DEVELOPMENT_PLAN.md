...
**Ver 2.2 - Backend Development Blueprint**
...
| 2.1 | 2026-01-17 | 카카오 OAuth, 일반 회원가입/로그인 추가 (프론트엔드 주도 방식) | MadCamp02 |
| **2.2** | **2026-01-17** | **OAuth2 백엔드 주도 방식으로 변경** | **MadCamp02** |

### Ver 2.2 주요 변경 사항

1. **OAuth2 방식 변경**: 프론트엔드 주도 → 백엔드 주도 (Spring Security OAuth2 Client)
2. **신규 파일 생성**: `OAuth2SuccessHandler.java`
3. **설정 파일 수정**: `application.yml`, `SecurityConfig.java`, `docker-compose.yml`
4. **리다이렉트 URI 변경**: 백엔드 URL로 통일
...
|------|------|------|------|
| Java | 21 LTS | 언어 | ✅ 사용중 |
| Spring Boot | 4.0.1 | 프레임워크 | ✅ 사용중 |
| Spring Security | 6.x | 인증/인가 | ✅ 사용중 |
| **Spring OAuth2 Client** | 6.x | **백엔드 주도 OAuth2 인증** | ✅ 사용중 |
| Spring Data JPA | 3.x | ORM | ✅ 사용중 |
| Spring WebSocket | 6.x | 실시간 통신 | 🔜 예정 |
| Spring Validation | 3.x | 입력 검증 | ✅ 사용중 |
| Spring Actuator | 3.x | 헬스체크/모니터링 | ✅ 사용중 |
| Flyway | Latest | DB 마이그레이션 | ✅ 사용중 |
| Lombok | Latest | 보일러플레이트 제거 | ✅ 사용중 |
| MapStruct | Latest | DTO 매핑 | 🔜 예정 |
| jjwt | 0.12.x | JWT 처리 | ✅ 사용중 |
| Lettuce | 6.x | Redis 클라이언트 | ✅ 사용중 |
| Google API Client | 2.2.0 | Google ID Token 검증 | ✅ 사용중 |
| RestTemplate | - | Kakao API 호출 | ✅ 사용중 |
...
│   │
│   ├── 📁 config/                        # 설정 클래스
│   │   ├── SecurityConfig.java           # ✅ Spring Security + OAuth2 Login 설정
│   │   │                                 #    - JWT 필터, CORS, OAuth2 성공 핸들러 연결
│   │   ├── AppConfig.java                # ✅ 🆕 공통 Bean 설정 (RestTemplate 등)
│   │   ├── WebSocketConfig.java          # 🔜 WebSocket/STOMP 설정
│   │   ├── RedisConfig.java              # ✅ Redis 연결 설정
│   │   ├── JpaConfig.java                # 🔜 JPA 상세 설정
│   │   ├── CorsConfig.java               # ✅ CORS 설정 (SecurityConfig 내 통합)
...
│   │   ├── JwtAuthenticationFilter.java  # ✅ 요청마다 JWT 토큰 검증 필터
│   │   ├── OAuth2SuccessHandler.java     # ✅ 🆕 백엔드 주도 OAuth2 성공 핸들러
│   │   │                                 #    - OAuth2 로그인 성공 시 JWT 발급
│   │   │                                 #    - 프론트엔드로 토큰과 함께 리다이렉트
│   │   ├── CustomUserDetails.java        # ✅ Spring Security 사용자 상세 정보
...
### 7.3 OAuth 로그인 흐름 (Google / Kakao) - 백엔드 주도 방식
...
└────┬────┘    └───────┬────────┘    └────┬────┘         └────┬────┘
     │                 │                   │                   │
     │ GET /oauth2/authorization/kakao (또는 google)           │
     │────────────────────────────────────▶│                   │
     │                 │                   │                   │
     │     OAuth2 인증 페이지 리다이렉트    │                   │
     │◀────────────────────────────────────│                   │
     ...
     │◀────────────────│                   │                   │
     │                 │                   │                   │
     │  GET /login/oauth2/code/kakao?code=xxx                  │
     │────────────────────────────────────▶│                   │
     │                 │                   │                   │
     │                 │  Access Token 교환 │                   │
     │                 │◀──────────────────│                   │
     ...
     │                 │                   │──────────────────▶│
     │                 │                   │                   │
     │  리다이렉트: http://localhost:3000/oauth/callback       │
     │  ?accessToken=xxx&refreshToken=xxx&isNewUser=true       │
     │◀────────────────────────────────────│                   │
```
...
│   └── /login/oauth2/**     # OAuth2 콜백 (리다이렉트 URI)
├── OAuth2 Login 설정 (백엔드 주도):
│   ├── successHandler: OAuth2SuccessHandler (JWT 발급 후 프론트로 리다이렉트)
│   └── failureHandler: 에러 JSON 응답
├── 보호 엔드포인트: 나머지 전체
└── JWT 필터 추가
```
...
### D. OAuth2 백엔드 주도 방식 (Ver 2.2 추가)

#### 방식 변경 이유

| 항목 | 프론트엔드 주도 (이전) | 백엔드 주도 (현재) |
|------|---------------------|------------------|
| **토큰 교환** | 프론트에서 Access Token 획득 후 백엔드로 전송 | 백엔드가 직접 Authorization Code → Token 교환 |
| **보안** | Access Token이 프론트엔드에 노출 | Authorization Code만 URL에 노출 (일회성) |
| **구현 복잡도** | 프론트/백 모두 구현 필요 | 백엔드에서 통합 관리 |
| **리다이렉트 URI** | 프론트엔드 URL | 백엔드 URL |

#### 신규 생성 파일

| 파일 | 경로 | 용도 |
|------|------|------|
| **OAuth2SuccessHandler.java** | `security/` | OAuth2 로그인 성공 시 JWT 발급 + 프론트엔드 리다이렉트 |
| **AppConfig.java** | `config/` | RestTemplate Bean 등록 (Kakao API 호출용) |

#### 수정된 파일

| 파일 | 변경 내용 |
|------|----------|
| `application.yml` | Kakao OAuth2 Client 설정 추가, 프론트엔드 리다이렉트 URI 설정 |
...
```
┌──────────────┐     ┌──────────────┐     ┌──────────────┐     ┌──────────────┐
│   Frontend   │     │   Backend    │     │    Kakao     │     │   Database   │
└──────┬───────┘     └──────┬───────┘     └──────┬───────┘     └──────┬───────┘
       │                    │                    │                    │
       │ 1. 카카오 로그인 버튼 클릭               │                    │
       │ ──────────────────▶│                    │                    │
       ...
       │                    │◀──────────────────────────────────────│
       │                    │                    │                    │
       │                    │ 8. JWT 발급 (OAuth2SuccessHandler)     │
       │                    │                    │                    │
       │ 9. 프론트엔드로 리다이렉트               │                    │
       │◀────────────────────                   │                    │
       │   (http://localhost:3000/oauth/callback?accessToken=xxx&refreshToken=xxx)
       │                    │                    │                    │
       │ 10. 토큰 저장 후 대시보드 이동           │                    │
       │                    │                    │                    │
```
...
```
📍 리다이렉트 URI 등록 (필수):
   - http://localhost:8080/login/oauth2/code/kakao (개발)
   - https://api.madcamp02.com/login/oauth2/code/kakao (운영)
