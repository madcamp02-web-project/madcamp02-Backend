 ...
 * Spring의 규칙: 클래스에 생성자가 딱 하나만 있으면, 스프링이 알아서 그 생성자를 통해 빈(Bean)을 주입해 줍니다.
 * (그래서 @Autowired도 생략 가능)
 * final의 안전함: private final로 선언했기 때문에, 이 변수는 생성될 때 딱 한 번만 값이 채워지고 절대 바뀌지
 * 않습니다(불변성).
...
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
                ...
                // 스프링 시큐리티의 기본 로그인 필터(UsernamePassword...)가 동작하기 전에 우리가 만든
